...
public class HightLightView extends FrameLayout {
    ...
    private List<HighLight.ViewPosGroup> mViewGroups;
    private HighLight mHighLight;
    private LayoutInflater mInflater;
    private NextBtnClick nextBtnClick;
...
//    private final boolean isNext;//next模式标志
    ...
    private List<HighLight.ViewPosInfo> mViewPosInfos;//当前显示的高亮布局位置信息

    public HightLightView(Context context, HighLight highLight, int maskColor, List<HighLight.ViewPosGroup> viewPosGroups, boolean isNext) {
        super(context);
        mHighLight = highLight;
        mInflater = LayoutInflater.from(context);
        mViewGroups = viewPosGroups;
...
//        if (isNext) {
        //校验mPosition
        if (mPosition < -1 || mPosition > mViewGroups.size() - 1) {
            //重置位置
            mPosition = 0;
        } else if (mPosition == mViewGroups.size() - 1) {
            //移除当前布局
            mHighLight.remove();
            return;
        } else {
        ...
        mViewPosInfos = mViewGroups.get(mPosition).subRects;
        //移除所有tip再添加当前位置的tip布局
        removeAllTips();
        for (HighLight.ViewPosInfo viewPosInfo : mViewPosInfos) {
...
//            }
        if (viewPosInfo.marginInfo.gravity == Gravity.NO_GRAVITY) {
            if (lp.rightMargin != 0) {
            ...
            lp.gravity = viewPosInfo.marginInfo.gravity;
        }
        ...
        View nextBtn = view.findViewWithTag("next_btn");
        if (nextBtn != null) {
            nextBtn.setOnClickListener(v -> {
                if (nextBtnClick != null) {
                    nextBtnClick.onNextBtnClick(v);
                }
            });
        }
        addView(view, lp);
...
//        } else {
        for (int i = 0, n = getChildCount(); i < n; i++) {
            ...
            HighLight.ViewPosInfo viewPosInfo = mViewPosInfos.get(i);
...
//    }
    ...
    public NextBtnClick getNextBtnClick() {
        return nextBtnClick;
    }

    public void setNextBtnClick(NextBtnClick nextBtnClick) {
        this.nextBtnClick = nextBtnClick;
    }

    public interface NextBtnClick {
